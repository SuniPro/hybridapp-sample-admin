...
public class DynamicSecurityServiceImpl implements DynamicSecurityService {
	...
	public String setSessionInfo(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			/* 관리자 부가정보 설정 */
			EgovMap mngrInfoMap = null;
					
				
			targetUrl = request.getRequestURL().toString().replace(request.getRequestURI(),"")+"/";
			mngrInfoMap = dynamicSecurityDAO.selectSecurityMngrInfo(userName);
			session.setAttribute(CmmnConstant.SESSION_MNGR, mngrInfoMap);
			...
			while(iter.hasNext()) {
				...
				// 권한을 2개 이상 사용하게 되면 List clone 구성하는 걸로... 
